...
void buffer::read(const std::filesystem::path &file_path) {
  ...
  if (std::filesystem::file_size(file_path) == 0) {
    ...
    BUFFER
        << "  file size is 0. buffer is filled with one line and one null byte."
        << '\n';
#endif
  ...
  BUFFER << "  file is read. buffer is filled with the lines of the file."
         ...
         << std::format("  filename: {}",
                        std::string(std::get<1>(metadata_).value_or("nullopt")))
         << '\n'
         << std::format("  ident: {}",
                        std::get<2>(metadata_).value_or("nullopt"))
         << '\n'
...
void buffer::read(const std::filesystem::path &file_path, const int &fd,
                  const off_t &file_size) {

#if DEBUG_BUFFER == true
  BUFFER << std::format("buffer::read(fd[{}], file_size[{}]) called ⇣", fd,
                        file_size)
         << '\n';
#endif
  ...
  // Ensure at least one element is in the buffer if no newline-separated
